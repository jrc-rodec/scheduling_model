...
namespace ReactionStrategy
{
    ...
    public class Individual : IEquatable<Individual> //: Individual
    {
        ...
        public static bool[] FixedOperations;
        ...
        public Individual(int[] sequence, int[] assignments, int[] workers, float p, double rate = 0.0)
        {
            _sequence = new int[sequence.Length];
            sequence.CopyTo(_sequence, 0);
            _assignments = new int[assignments.Length];
            assignments.CopyTo(_assignments, 0);
            _workers = new int[workers.Length];
            workers.CopyTo(_workers, 0);
            double mutation = 1.0f;
            Random random = new Random();
            while(mutation > random.NextDouble())
            {
                Mutate(p);
                mutation *= rate;
            }
