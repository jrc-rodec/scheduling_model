...
\begin{itemize}
	...
	\item resources – (list) needed resources, to process the task (id + amonut)
...
\begin{itemize}
	\item id – unique ID of the order (just used for internal identiciation)
	\item arrival\_time – timestamp, when the order was made
	\item delivery\_time – timestamp, when the order is supposed to be done
	\item latest\_acceptable\_time – timestampe, latest acceptable point in time to which the order needs to be completed if the given delivery time can not be met
	...
	\item divisible – boolean, to flag if the order can be split and only partially fullfilled or if all of the ordered resources have to be done (or none)
...
\subsection{Used Objective Functions}
...
In Stage 2, every Job is represented by a single integer <start time> - the workstation assignments needed for the fitness function are taken of the result
of Stage 1
